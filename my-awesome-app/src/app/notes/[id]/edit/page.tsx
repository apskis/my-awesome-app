...
export default function EditNotePage() {
  ...
  // Fetch note, categories, and tags on component mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [noteResponse, categoriesResponse, tagsResponse] = await Promise.all([
  ...
  const handleAddTag = async (tag: Tag) => {
    if (!selectedTags.find(t => t.id === tag.id)) {
      try {
        const response = await fetch(`/api/notes/${params.id}/tags`, {
          method: 'POST',
          headers: {
  ...
  const handleRemoveTag = async (tagId: string) => {
    try {
      const response = await fetch(`/api/notes/${params.id}/tags/${tagId}`, {
        method: 'DELETE',
      })
